...
export default function Reports() {
  ...
  const [typeTab, setTypeTab] = useState("idea"); // "idea" | "learning_material"
  ...
  const fetchReports = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (isAdmin || isQAC) params.type = typeTab;
      const { data } = await getAllReports(params);
      setReports(data.reports || []);
      setPagination({ ...data.pagination, page });
    } catch { setReports([]); }
    finally { setLoading(false); }
  }, [filterStatus, typeTab]);

  useEffect(() => { fetchReports(1); }, [fetchReports]);

  // Reset to page 1 when switching tabs
  useEffect(() => { setFilterStatus(""); setExpanded(null); }, [typeTab]);

  // Auto-scroll and expand the highlighted report from notification
  useEffect(() => {
    if (!highlightId || loading || reports.length === 0) return;
    // Auto-expand the highlighted report
    setExpanded(highlightId);
    // Scroll to it after a short delay (let DOM render)
    setTimeout(() => {
      const el = reportRefs.current[highlightId];
  ...
  return (
    <Layout>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        {/* Type Tabs — Admin and QAC */}
        {(isAdmin || isQAC) && (
          <div className="flex gap-2 mb-5 border-b border-slate-200">
            {[
              { key: "idea", label: "💡 Idea Reports" },
              { key: "learning_material", label: "📚 Learning Material Reports" },
            ].map(tab => (
              <button
                key={tab.key}
                onClick={() => setTypeTab(tab.key)}
                className={`px-5 py-2.5 text-sm font-bold border-b-2 transition-all ${typeTab === tab.key ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-800"}`}
              >
                {tab.label}
              </button>
            ))}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-24 text-slate-400">
            ...
            {isQAC && <p className="text-sm mt-1">Reports you submit will appear here.</p>}
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {reports.map(report => {
              ...
              return (
                <div
                  ...
                  className={`border rounded-xl overflow-hidden shadow-sm transition-all duration-500 ${
                    highlightId === report._id
                      ? "bg-yellow-50 border-yellow-400 shadow-yellow-200 shadow-md ring-2 ring-yellow-300"

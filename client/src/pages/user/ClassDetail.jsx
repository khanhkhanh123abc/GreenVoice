...
export default function ClassDetail() {
  ...
  const handleSubmitFeedback = async () => {
    if (!rating) {
      setFeedbackResult({ type: "error", msg: "Please select a star rating." });
      return;
    }
    try {
      setSubmitting(true);
      ...
      setFeedbackResult({
        type: "success",
        msg: "Feedback submitted successfully! You can rate again next month.",
      });
      setRating(0);
      setComment("");
      fetchMyFeedback();
      ...
      setFeedbackResult({
        type: "error",
        msg: err.response?.data?.message || "Failed to submit feedback.",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  if (!cls) {
    return (
      <Layout>
        <div className="p-8 text-center text-slate-500">Class not found.</div>
      </Layout>
    );
  }
    ...
    <Layout>
      <div className="p-8 max-w-4xl mx-auto">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-6 text-white mb-6">
          ...
          {cls.academicStaff && (
            <div className="mt-4 pt-4 border-t border-white/20 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-blue-200">Assigned Teacher</p>
        ...
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1 mb-6">
          {[
            { key: "members", label: "Class Members", icon: <Users size={15} /> },
            ...(user.role === "Student" && cls.academicStaff
              ? [{ key: "feedback", label: "Rate Teacher", icon: <Star size={15} /> }]
              : []),
          ].map((tab) => (
            <button
              key={tab.key}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {cls.students?.length === 0 ? (
                <p className="text-slate-400 text-sm py-4 text-center">No students yet</p>
              ) : (
            ...
            {!alreadyFeedbackThisMonth ? (
              <div className="bg-white border border-slate-200 rounded-2xl p-6">
                <h3 className="font-bold text-slate-900 mb-1">
                  Rate Teacher This Month
                </h3>
                <p className="text-sm text-slate-500 mb-5">
                  You can submit 1 rating per month for{" "}
                ...
                <div className="mb-5">
                  ...
                  <textarea
                    ...
                    rows={4}
                    maxLength={2000}
                    placeholder="Enter your comment about the teacher..."
              ...
              <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-green-800">
                    You have already rated the teacher this month!
                  </p>
                  <p className="text-sm text-green-600">
                    You can rate again next month.
                  </p>
                </div>
              </div>
            )}

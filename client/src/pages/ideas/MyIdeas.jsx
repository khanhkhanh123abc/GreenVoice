...
export default function MyIdeas() {
  ...
  const fetchMyIdeas = useCallback(async (page = 1) => {
    ...
    try {
      ...
      // Send own ID so Backend can filter
      params.set("author", user._id || user.id); 
      params.set("sortBy", "latest"); // Always sort newest first
      ...
      setIdeas([]);
      toast.error("Failed to load your idea list.");
    } finally {
      setLoading(false);
    }
  }, [searchQ, user]);
  ...
  // 🚀 REAL-TIME LISTENER (Keep for smooth number updates)
  useEffect(() => {
    socket.on("global_update_likes", (data) => {
      setIdeas(prev => prev.map(item =>
        item._id?.toString() === data.ideaId?.toString()
      ...
      <main className="flex-1 p-8 overflow-y-auto bg-slate-50">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-xl font-bold text-slate-800">My Ideas ({pagination.total})</h2>
          </div>
          ...
          <div className="flex flex-col rounded-xl overflow-hidden border border-slate-200 shadow-sm">
            {loading ? (
              ...
              <div className="text-center py-16 bg-white flex flex-col items-center">
                ...
                <div className="text-slate-500 font-medium mb-4">You have not posted any ideas yet.</div>
                <button onClick={() => navigate('/submit-idea')} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 transition-colors">Post Now</button>
              </div>
            ) : ideas.map((idea, idx) => (
              <IdeaCard
                key={idea._id}
                ...
                canEdit={true} // Own ideas can always be edited
                canDelete={true} // Own ideas can always be deleted
...
function IdeaCard({ idea, tc, isLast, canEdit, canDelete, onView, onEdit, onDelete }) {
  ...
  const isLiked = (likes) => likes?.some(id => id?.toString() === currentUserId?.toString()) || false;
  const [liked, setLiked] = useState(isLiked(idea.likes));
  const [likeCount, setLikeCount] = useState(idea.votes || 0);

  useEffect(() => {
    setLiked(isLiked(idea.likes));
    ...
    <div className={`flex bg-white hover:bg-slate-50 transition-colors cursor-pointer ${!isLast ? 'border-b border-slate-200' : ''}`} onClick={onView}>
      <div className="flex-1 p-5 min-w-0">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {idea.status === "pending" && (
              <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-orange-100 text-orange-700 border border-orange-200 flex items-center gap-1">
                ⏳ Pending review
              </span>
            )}

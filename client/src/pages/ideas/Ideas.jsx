...
import { Heart, MessageCircle, Eye, Flag, AlertCircle, X } from "lucide-react";
...
export default function Ideas() {
  ...
  const [modal, setModal] = useState(null); // "edit" | "delete" | "report"
  ...
  const fetchIdeas = useCallback(async (page = 1) => {
    ...
    try {
      ...
      // 1. If search query is entered
      if (searchQ) params.set("search", searchQ);

      // 2. If a Category is selected (other than "All Categories")
      if (filterCategory !== "All Categories") {
        params.set("category", filterCategory);
      }

      // 3. HANDLE TABS (Pass sort order to Backend)
      if (activeTab === "Most Popular") params.set("sortBy", "popular"); // Sort by votes
      if (activeTab === "Most Viewed") params.set("sortBy", "viewed");   // Sort by views
      if (activeTab === "Latest Ideas") params.set("sortBy", "latest");  // Sort by creation date (Newest)
      if (activeTab === "Most Comments") params.set("sortBy", "comments"); // Sort by comments
  ...
  }, [searchQ, filterCategory, activeTab]); // <--- Put state in dependency array

  // Re-call API immediately (and go to page 1) whenever user switches Tab or Category
  useEffect(() => {
    fetchIdeas(1);
  ...
  const openReportModal = (idea) => {
    if (reportedIds.has(idea._id)) {
      ...
      {modal === "report" && (
        <Overlay onClose={closeModal}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-slate-900">Report Idea</h2>
            <button onClick={closeModal} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
              <X size={18} className="text-slate-500" />
            </button>
          </div>
          <p className="text-sm text-slate-500 mb-4">Material: <span className="text-blue-600 font-medium">{selected?.title}</span></p>
          <form onSubmit={handleSubmitReport} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-1">
              <button type="button" onClick={closeModal} className="px-5 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors">Cancel</button>
              <button type="submit" disabled={formLoading} className="px-5 py-2 text-sm font-semibold text-white bg-red-600 rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors">
...
// ─────────────────────────────────────────────
// OPTIMIZED IDEACARD COMPONENT
// ─────────────────────────────────────────────
function IdeaCard({ idea, tc, isLast, canEdit, canDelete, isQAC, reportedIds, onView, onEdit, onDelete, onReport }) {
  const { user } = useAuth();
  ...
  // 🚀 STATE SYNC: Auto-update if data (from Socket) changes
  useEffect(() => {
  ...
  const handleLike = async (e) => {
    ...
    // Save old values as fallback (Optimistic UI)
    const originalLiked = liked;
    const originalCount = likeCount;

    // Immediately update count for smooth UX
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-1.5 text-slate-400 hover:text-blue-500 transition-colors">
              {/* FIX: Use commentsCount from Socket, fallback to array length */}
          ...
          <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
            {isQAC && (
              <button
                ...
                className={`p-1.5 rounded-lg transition-colors ${reportedIds?.has(idea._id) ? 'text-slate-300 cursor-default' : 'text-slate-400 hover:text-red-600 hover:bg-red-50'}`}
                title={reportedIds?.has(idea._id) ? "Already reported" : "Report this idea"}
              >
                <Flag size={16} />
              </button>
            )}

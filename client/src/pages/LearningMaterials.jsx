...
import {
  ...
  Archive, ArchiveRestore, X, Upload, ChevronLeft, ChevronRight
} from "lucide-react";
import {
...
// ── MATERIAL CARD ──
function MaterialCard({ mat, user, isOwner, isAdmin, onView, onEdit, onDelete, onArchive, onLikeUpdate }) {
      ...
      <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto" onClick={e => e.stopPropagation()}>
        ...
        {(isOwner || isAdmin) && (
          <div className="flex items-center gap-1">
            ...
            {isAdmin && (
              <button onClick={(e) => { e.stopPropagation(); onArchive(); }} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title={isArchived ? "Restore" : "Archive"}>
...
function ModalActions({ onCancel, loading, label }) {
  return (
    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
      ...
      <button type="submit" disabled={loading} className="px-5 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors">
